...
namespace EvidenceZakazekWebApp.Extensions
{
    public static class HtmlHelperExtensions
    {
        ...
        public static IDisposable BeginCollectionItemWithGuid(this HtmlHelper html, string collectionName, string Guid)
        {
            var idsToReuse = GetIdsToReuse(html.ViewContext.HttpContext, collectionName);
            string itemIndex = idsToReuse.Count > 0 ? idsToReuse.Dequeue() : Guid;
            ...
            html.ViewContext.Writer.WriteLine(string.Format("<input " +
              "type=\"hidden\" name=\"{0}.index\" autocomplete=\"off\" value =\"{1}\" />",
              !string.IsNullOrEmpty(previousPrefix) ? string.Concat(previousPrefix.Trim('.'), ".", collectionName) :
